...
def mkdir(path):
    ...
    isExists = os.path.exists(os.path.join("/Users/jerry/jiandan", path))
    # 如果不存在
    if not isExists:
        print('makedir', path)
        # 创建文件夹
        os.makedirs(os.path.join("/Users/jerry/jiandan", path))
        # 切换到创建的文件夹
        os.chdir(os.path.join("/Users/jerry/jiandan", path))
        return True
    # 如果存在了就返回False
    else:
...
def all_page():
    ...
    for page in range(1, 2):
        ...
        midurl=date + '-' + str(page)
        allurl = base_url + str(base64.b64encode(midurl.encode('utf-8')),'utf-8') + '#comments'  # 拼出分页地址
...
if __name__ == '__main__':
    # 计时
    t1 = time.time()
    mkdir(time.strftime('%Y%m%d',time.localtime(time.time())))
    # 调用函数
    get_imgs()
    print(time.time() - t1)